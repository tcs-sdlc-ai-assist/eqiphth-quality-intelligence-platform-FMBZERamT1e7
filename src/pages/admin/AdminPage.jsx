...
import {
  getAllPersonas,
  getAllSegments as getAllPersonaSegments,
  getAllRoles,
} from '@/data/personas';
import {
  getAllSegments,
  getSegmentAggregates,
} from '@/data/segments';
import {
  getAllIntegrations,
...
import {
  getAuditLogAggregates,
  getAllAuditLogEventTypes,
} from '@/data/auditLogs';
...
function CustomTooltipChart({ active, payload, label, unit = '' }) {
...
function CreateEditUserDialog({ open, onOpenChange, onSubmit, loading, user }) {
  const isEdit = Boolean(user);

  const roleOptions = useMemo(() => {
    return getAllRoles().map((r) => ({ value: r, label: r }));
  }, []);

  const segmentOptions = useMemo(() => {
    return getAllPersonaSegments().map((s) => ({ value: s, label: s }));
  }, []);
...
function UserDetailDialog({ user, open, onOpenChange, onLock, onUnlock, onDeactivate, onActivate }) {
...
function UserManagementTab({ users, onUserClick, onCreateClick, onEditClick, onExportCSV, onExportJSON }) {
  const { hasPermission } = usePersona();
  ...
  const tableColumns = useMemo(
    ...
    [onUserClick, onEditClick, hasPermission]
  );
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <PermissionGate requiredAction={PERMISSIONS.MANAGE_USERS} behavior="hidden">
            <Button
              variant="primary"
              size="sm"
...
function RoleManagementTab() {
  ...
  const segments = useMemo(() => getAllPersonaSegments(), []);
  ...
  return (
    <div className="flex flex-col gap-6">
      <PanelCard
        title="Role Overview"
        subtitle={`${roles.length} unique roles across ${segments.length} segments`}
        icon={<Shield className="h-5 w-5" />}
      >
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Total Roles</span>
            <span className="text-2xl font-semibold text-slate-900">{roles.length}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Total Personas</span>
            <span className="text-2xl font-semibold text-slate-900">{personas.length}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Segments</span>
            <span className="text-2xl font-semibold text-slate-900">{segments.length}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Avg Permissions</span>
            <span className="text-2xl font-semibold text-slate-900">
              {personas.length > 0
                ? Math.round(personas.reduce((sum, p) => sum + p.permissions.length, 0) / personas.length)
                : 0}
            </span>
          </div>
        </div>
          ...
          <ChartWrapper height={250} noCard noPadding>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={segmentData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="count" name="Personas" radius={[4, 4, 0, 0]} barSize={32}>
      ...
      <PanelCard
        ...
        subtitle="All configured personas with permissions"
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className="text-sm font-medium text-slate-900">{persona.name}</span>
                    <span className="text-2xs text-slate-500">{persona.role} • {persona.segment}</span>
                  </div>
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {persona.permissions.slice(0, 5).map((perm) => (
                    <Badge key={perm} variant="outline" size="sm">{perm}</Badge>
                  ))}
                  {persona.permissions.length > 5 ? (
                    <Badge variant="outline" size="sm">+{persona.permissions.length - 5}</Badge>
                  ) : null}
                </div>
              </div>
            </div>
...
function SegmentConfigTab() {
  const segmentsData = useMemo(() => getAllSegments(), []);
  const segmentAggregates = useMemo(() => getSegmentAggregates(), []);

  return (
    <div className="flex flex-col gap-6">
      <PanelCard
        title="Segment Configuration"
        subtitle={`${segmentsData.length} segments configured`}
        icon={<Layers className="h-5 w-5" />}
      >
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Total Segments</span>
            <span className="text-2xl font-semibold text-slate-900">{segmentsData.length}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Total Applications</span>
            <span className="text-2xl font-semibold text-slate-900">{segmentAggregates.totalApplications}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Avg Quality</span>
            <span className="text-2xl font-semibold text-slate-900">{segmentAggregates.averageQualityScore.toFixed(1)}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Avg Compliance</span>
            <span className="text-2xl font-semibold text-slate-900">{segmentAggregates.averageComplianceRate.toFixed(1)}%</span>
          </div>
        </div>
      </PanelCard>
          ...
          <Card key={segment.id} className="p-5">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="flex flex-col gap-0 min-w-0">
                  ...
                  <span className="text-2xs text-slate-500">{segment.owner}</span>
                </div>
              </div>
              <StatusPill status={segment.status} size="sm" dot />
            </div>

            <p className="mt-2 text-xs text-slate-500 line-clamp-2">{segment.description}</p>

            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="flex flex-col gap-0.5">
                <span className="text-2xs text-slate-400">Quality</span>
                <span className="text-sm font-semibold text-slate-900">{segment.qualityScore.toFixed(1)}</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <span className="text-2xs text-slate-400">Compliance</span>
                <span className="text-sm font-semibold text-slate-900">{segment.complianceRate.toFixed(1)}%</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <span className="text-2xs text-slate-400">Applications</span>
                <span className="text-sm font-semibold text-slate-900">{segment.applicationCount}</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <span className="text-2xs text-slate-400">Releases</span>
                <span className="text-sm font-semibold text-slate-900">{segment.releaseCount}</span>
              </div>
            </div>
            ...
            {segment.applications && segment.applications.length > 0 ? (
              <div className="mt-2.5 flex flex-wrap gap-1">
                {segment.applications.slice(0, 3).map((appId) => (
                  <Badge key={appId} variant="outline" size="sm">{appId}</Badge>
                ))}
                {segment.applications.length > 3 ? (
                  <Badge variant="outline" size="sm">+{segment.applications.length - 3}</Badge>
                ) : null}
              </div>
            ) : null}
          </Card>
...
function PlatformConfigTab() {
  const integrationAggregates = useMemo(() => getIntegrationAggregates(), []);
  const environmentAggregates = useMemo(() => getEnvironmentAggregates(), []);
  const auditAggregates = useMemo(() => getAuditLogAggregates(), []);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          label="Integrations"
          value={integrationAggregates.totalIntegrations}
          unit="count"
          trend="stable"
          status="on_track"
          description={`${integrationAggregates.activeCount} active, ${integrationAggregates.errorCount} errors`}
        />
        <KpiCard
          label="Environments"
          value={environmentAggregates.totalEnvironments}
          unit="count"
          trend="stable"
          status="on_track"
          description={`${environmentAggregates.availableCount} available, ${environmentAggregates.downCount} down`}
        />
        <KpiCard
          label="Audit Events"
          value={auditAggregates.totalLogs}
          unit="count"
          trend="improving"
          status="on_track"
          description={`${auditAggregates.successCount} success, ${auditAggregates.failureCount} failures`}
        />
        <KpiCard
          label="Avg Env Health"
          value={environmentAggregates.averageHealthScore}
          unit="percent"
          trend={environmentAggregates.averageHealthScore >= 90 ? 'improving' : 'stable'}
          status={environmentAggregates.averageHealthScore >= 90 ? 'on_track' : 'at_risk'}
          description="Average environment health score"
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <PanelCard
          ...
          subtitle="Configure platform notification delivery"
          icon={<Bell className="h-5 w-5" />}
        >
          <div className="flex flex-col gap-4">
            <Switch
              label="Email Notifications"
              description="Send notifications via email to configured recipients"
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                setNotificationSettings((prev) => ({ ...prev, teamsEnabled: checked }))
              }
            />
            <Switch
              label="In-App Notifications"
              description="Show notifications within the platform UI"
            ...
            />
            <Switch
              label="Priority Queue Processing"
              description="Process high-priority notifications before standard ones"
              checked={notificationSettings.priorityQueue}
              onCheckedChange={(checked) =>
                setNotificationSettings((prev) => ({ ...prev, priorityQueue: checked }))
              }
            />
            <div className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-slate-700">Batch Size</span>
              <span className="text-2xs text-slate-500">Current: {notificationSettings.batchSize} notifications per batch</span>
              <Progress
        ...
        <PanelCard
          ...
          subtitle="Configure data retention and archival policies"
          icon={<Database className="h-5 w-5" />}
        >
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-slate-200 p-3">
                <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Retention Period</span>
                <p className="mt-1 text-lg font-semibold text-slate-900">{dataSettings.retentionDays} days</p>
              </div>
              <div className="rounded-lg border border-slate-200 p-3">
                <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Backup Frequency</span>
                <p className="mt-1 text-lg font-semibold text-slate-900 capitalize">{dataSettings.backupFrequency}</p>
              </div>
            </div>
            <Switch
              label="Auto-Archive"
              description="Automatically archive data older than retention period"
              checked={dataSettings.autoArchive}
              onCheckedChange={(checked) =>
                setDataSettings((prev) => ({ ...prev, autoArchive: checked }))
              }
            />
            <Switch
              label="Compression"
              description="Enable data compression for archived records"
              checked={dataSettings.compressionEnabled}
              onCheckedChange={(checked) =>
                setDataSettings((prev) => ({ ...prev, compressionEnabled: checked }))
              }
            />
          </div>
        </PanelCard>
      ...
      <PanelCard
        ...
        subtitle="System-wide health and performance metrics"
        icon={<Activity className="h-5 w-5" />}
      >
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Active Integrations</span>
            <div className="flex items-center gap-1.5">
              <Wifi className="h-4 w-4 text-success-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{integrationAggregates.activeCount}</span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Integration Errors</span>
            <div className="flex items-center gap-1.5">
              <XCircle className="h-4 w-4 text-danger-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{integrationAggregates.integrationsWithErrors}</span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Envs Available</span>
            <div className="flex items-center gap-1.5">
              <Server className="h-4 w-4 text-success-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{environmentAggregates.availableCount}</span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Envs Down</span>
            <div className="flex items-center gap-1.5">
              <XCircle className="h-4 w-4 text-danger-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{environmentAggregates.downCount}</span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Audit Success</span>
            <div className="flex items-center gap-1.5">
              <CheckCircle className="h-4 w-4 text-success-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{auditAggregates.successCount}</span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">Audit Denied</span>
            <div className="flex items-center gap-1.5">
              <AlertTriangle className="h-4 w-4 text-warning-500" aria-hidden="true" />
              <span className="text-2xl font-semibold text-slate-900">{auditAggregates.deniedCount}</span>
            </div>
          </div>
        </div>

        <div className="mt-4 pt-4 border-t border-slate-200">
          <h4 className="text-sm font-semibold text-slate-900 mb-2">Environment Health</h4>
          <Progress
            value={environmentAggregates.averageHealthScore}
            max={100}
            variant="auto"
        ...
        <div className="mt-4 pt-4 border-t border-slate-200">
          <h4 className="text-sm font-semibold text-slate-900 mb-2">Integration Health</h4>
          <Progress
            value={integrationAggregates.activeCount}
            max={integrationAggregates.totalIntegrations || 1}
            variant="auto"
            size="md"
            showValue
        ...
        <div className="mt-4 pt-4 border-t border-slate-200">
          <h4 className="text-sm font-semibold text-slate-900 mb-2">Audit Event Types</h4>
          <div className="flex flex-col gap-2">
            {Object.entries(auditAggregates.eventTypeBreakdown || {})
              .sort((a, b) => b[1] - a[1])
              .slice(0, 8)
              .map(([eventType, count]) => (
                <div key={eventType} className="flex items-center gap-3">
                  <div className="w-32 shrink-0 truncate">
                    <span className="text-sm font-medium text-slate-700">{formatLabel(eventType)}</span>
                  </div>
                  <div className="flex-1">
                    <Progress
                      value={count}
                      max={auditAggregates.totalLogs || 1}
                      variant="primary"
                      size="xs"
                      animate
                    />
...
function UserAnalyticsTab({ users }) {
...
function AdminPageSkeleton() {
  return (
    <div className="flex flex-col gap-6" role="status" aria-label="Loading administration" aria-busy="true">
 ...
 * Provides user management, role management, segment configuration,
 * platform configuration, and analytics. Includes tabs for each admin
 * area with CRUD actions (simulated). All data from mockService.
 *
...
function AdminPage() {
    ...
    (formData) => {
      ...
      setTimeout(() => {
        if (editTarget) {
          ...
          logEvent('user_management', {
            action: 'User Created',
            details: `User "${formData.name}" created by ${currentPersona.name}.`,
  ...
  const handleExportCSV = useCallback(() => {
    try {
      ...
      logEvent('data_export', {
        ...
        details: `Users exported as CSV by ${currentPersona.name}. ${data.length} records.`,
        resource: '/settings',
        outcome: 'success',
      });
      toast({
  ...
  const handleExportJSON = useCallback(() => {
    try {
      ...
      logEvent('data_export', {
        action: 'Exported Users',
        details: `Users exported as JSON by ${currentPersona.name}. ${data.length} records.`,
        resource: '/settings',
        outcome: 'success',
      });
      toast({
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          ...
          <TabsTrigger value="segments">Segment Config</TabsTrigger>
