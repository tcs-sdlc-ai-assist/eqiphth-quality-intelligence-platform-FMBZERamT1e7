...
import { getAllPendingApprovals } from '@/data/agents';
...
function QuickActionsPanel() {
    ...
    const items = [
      {
        ...
        description: 'Browse all test execution results',
        icon: Play,
        color: '#16b364',
      ...
      },
      {
        ...
        description: 'Manage test execution schedules',
        icon: Calendar,
        color: '#3b82f6',
        permission: PERMISSIONS.VIEW_MEASURES,
      },
      {
        ...
        description: 'Browse test case repository',
        icon: TestTube,
        color: '#8b5cf6',
        permission: PERMISSIONS.VIEW_MEASURES,
      },
      {
        id: 'view_quality_gates',
        label: 'Quality Gates',
        description: 'Review quality gate status',
        icon: Shield,
        color: '#f59e0b',
        permission: PERMISSIONS.VIEW_QUALITY_GATES,
      },
    ];
  ...
  return (
    <PanelCard
      title="Quick Actions"
      subtitle="Common test orchestration actions"
        ...
        {actions.map((action) => {
          ...
          return (
            <div
              key={action.id}
              className={cn(
                'flex flex-col items-center gap-2 rounded-lg border border-slate-200 p-4 text-center transition-all duration-200',
            ...
            >
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-2xs text-slate-500">{action.description}</span>
              </div>
            </div>
          );
        })}
...
function AnalyticsPanel({ executions, schedules, automationData }) {
  ...
  const scheduleStatusData = useMemo(() => {
    ...
    for (const sched of schedules) {
      counts[sched.status] = (counts[sched.status] || 0) + 1;
    }
    return Object.entries(counts).map(([status, count]) => ({
      status,
      count,
  ...
  const scheduleFrequencyData = useMemo(() => {
    const counts = {};
    for (const sched of schedules) {
      counts[sched.frequency] = (counts[sched.frequency] || 0) + 1;
    }
  ...
  const healthScores = useMemo(() => {
    if (!automationData || !automationData.healthScores) return [];
    return automationData.healthScores
      .map((h) => ({
        name: h.applicationName.length > 18 ? h.applicationName.substring(0, 18) + '…' : h.applicationName,
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      ...
      <PanelCard
        ...
        subtitle="Schedule status and frequency distribution"
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">By Frequency</h4>
            <div className="flex flex-col gap-2">
              {scheduleFrequencyData.map((item) => (
                <div key={item.frequency} className="flex items-center gap-3">
                  <div className="w-20 shrink-0">
                    <Badge variant="outline" size="sm">{item.label}</Badge>
                  </div>
                  <div className="flex-1">
                    <Progress
                      value={item.count}
      ...
      {healthScores.length > 0 ? (
        <PanelCard
          title="Automation Health Scores"
          subtitle="Application automation health rankings"
          icon={<Activity className="h-5 w-5" />}
          className="xl:col-span-2"
        >
          <ChartWrapper height={280} noCard noPadding>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={healthScores} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis
                  dataKey="name"
                  tick={{ fontSize: 10, fill: '#64748b' }}
                ...
                <Bar dataKey="score" name="Health Score" radius={[4, 4, 0, 0]} barSize={24}>
                  {healthScores.map((entry) => {
                    let color = '#10b981';
                    if (entry.score < 60) color = '#ef4444';
                    else if (entry.score < 80) color = '#f59e0b';
...
function HTHHomeSkeleton() {
  return (
    <div className="flex flex-col gap-6" role="status" aria-label="Loading HTH Home" aria-busy="true">
...
function HTHHomePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [execData, schedData, hthData] = await Promise.all([
        getTestExecutions(filterParams),
        getSchedules(),
        getHTHMetrics(),
      ]);
  ...
  const kpiData = useMemo(() => {
    ...
    const avgDuration = total > 0
      ? Math.round(executions.filter((e) => e.duration > 0).reduce((sum, e) => sum + e.duration, 0) / Math.max(executions.filter((e) => e.duration > 0).length, 1))
      : 0;
    ...
    return [
      ...
      {
        id: 'kpi_active_schedules',
        label: 'Active Schedules',
        value: activeSchedules,
        unit: 'count',
        trend: 'stable',
        status: 'on_track',
        description: 'Number of active test execution schedules.',
      },
      {
        ...
        value: pendingCount,
        unit: 'count',
        trend: pendingCount > 3 ? 'declining' : 'stable',
        status: pendingCount > 5 ? 'at_risk' : 'on_track',
        description: 'Agent actions awaiting human approval.',
      },
    ];
  }, [executions, schedules, pendingApprovals]);
  ...
  const insightData = useMemo(() => {
    ...
    if (failed.length > 5) {
      return {
        ...
        message: `${failed.length} out of ${executions.length} test executions have failed status. ${withDefects.length} executions discovered defects. Review failed executions for root cause analysis.`,
    ...
    return {
      variant: 'success',
      title: 'Test orchestration performing well',
  ...
  const executionSummary = useMemo(() => {
    ...
    const avgConfidence = executions.length > 0
      ? Math.round(executions.reduce((sum, e) => sum + (e.aiAnalysis.confidence || 0), 0) / executions.length)
      : 0;

    return { passed, failed, blocked, skipped, inProgress, totalDefects, avgConfidence };
  }, [executions]);

  if (loading) {
    return <HTHHomeSkeleton />;
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold text-slate-900">Test Orchestration</h1>
          <p className="text-sm text-slate-500">
            HTH test execution dashboard, schedules, and automation intelligence for {currentPersona.name}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview">
          <div className="flex flex-col gap-6">
            ...
            <PanelCard
              ...
              subtitle="Overview of test execution results"
              icon={<Activity className="h-5 w-5" />}
            >
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-7">
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-2xs font-medium text-slate-400 uppercase tracking-wider">In Progress</span>
                  <span className="text-2xl font-semibold text-info-600">{executionSummary.inProgress}</span>
                </div>
                <div className="flex flex-col gap-1">
            ...
            <PanelCard
              ...
              subtitle="Latest test execution results"
            ...
            >
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-2xl font-semibold text-slate-900">{schedules.length}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 flex flex-col gap-2">
                {['daily', 'weekly', 'cron'].map((freq) => {
                  const count = schedules.filter((s) => s.frequency === freq).length;
                  return (
                    <div key={freq} className="flex items-center gap-3">
                      <div className="w-16 shrink-0">
                        <Badge variant="outline" size="sm" className="capitalize">{freq}</Badge>
                      </div>
                      <div className="flex-1">
                        <Progress
                          value={count}
            ...
            {pendingApprovals.length > 0 ? (
              <PanelCard
                ...
                subtitle={`${pendingApprovals.length} actions awaiting human review`}
              ...
              >
                <div className="flex flex-col gap-2">
                  {pendingApprovals.slice(0, 5).map((approval) => (
                    ...
                    >
                      <div
                        className="h-8 w-8 shrink-0 rounded-lg flex items-center justify-center bg-warning-50"
                      >
                        <AlertTriangle className="h-4 w-4 text-warning-500" aria-hidden="true" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-900 line-clamp-1">{approval.item.action}</p>
                        <p className="text-2xs text-slate-500 mt-0.5">{approval.agentName}</p>
                        <div className="mt-1 flex items-center gap-2">
                          <Badge
                            variant={
                              approval.item.priority === 'critical' ? 'error' :
                              approval.item.priority === 'high' ? 'warning' :
                              'info'
                            }
                            size="sm"
                          >
                            {approval.item.priority}
                          </Badge>
                          <span className="text-2xs text-slate-400">
                            {formatDate(approval.item.requestedAt, 'MMM d, h:mm a')}
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <TabsContent value="analytics">
          {executions.length === 0 && schedules.length === 0 ? (
            <EmptyState
              type="no_chart"
              title="No data for analytics"
              message="No test execution or schedule data available to generate analytics."
              size="lg"
              bordered
            />
          ) : (
